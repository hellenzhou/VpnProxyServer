/*
 * VPN服务器数据包转发器实现 - 重构版
 * 核心改进：
 * 1. 只转发payload（应用层数据），不转发完整IP包
 * 2. 使用NAT表管理连接状态
 * 3. 正确封装响应包
 */

#include "packet_forwarder.h"
#include "vpn_server_globals.h"
#include "simple_dns_cache.h"
#include "packet_builder.h"
#include "nat_table.h"
#include <hilog/log.h>
#include <thread>
#include <unistd.h>
#include <fcntl.h>
#include <errno.h>
#include <cstring>
#include <arpa/inet.h>
#include <sys/select.h>
#include <sys/time.h>
#include <sys/socket.h>

// 日志宏定义
#define MAKE_FILE_NAME (strrchr(__FILE__, '/') ? (strrchr(__FILE__, '/') + 1) : __FILE__)
#define VPN_SERVER_LOGE(fmt, ...) \
  OH_LOG_Print(LOG_APP, LOG_INFO, 0x15b1, "VpnServer", "[%{public}s:%{public}d] " fmt, MAKE_FILE_NAME, __LINE__, ##__VA_ARGS__)
#define VPN_SERVER_LOGI(fmt, ...) \
  OH_LOG_Print(LOG_APP, LOG_INFO, 0x15b1, "VpnServer", "[%{public}s:%{public}d] " fmt, MAKE_FILE_NAME, __LINE__, ##__VA_ARGS__)

// 转发数据包到真实服务器 - 重构版
int PacketForwarder::ForwardPacket(const uint8_t* data, int dataSize, 
                                  const PacketInfo& packetInfo, 
                                  const sockaddr_in& originalPeer) {
    VPN_SERVER_LOGI("📦 [转发-新] 开始: %{public}s:%{public}d -> %{public}s:%{public}d (%{public}s)",
                    packetInfo.sourceIP.c_str(), packetInfo.sourcePort,
                    packetInfo.targetIP.c_str(), packetInfo.targetPort,
                    ProtocolHandler::GetProtocolName(packetInfo.protocol).c_str());
    
    // 1. 从完整IP包中提取payload（应用层数据）
    const uint8_t* payload = nullptr;
    int payloadSize = 0;
    
    if (!PacketBuilder::ExtractPayload(data, dataSize, packetInfo, &payload, &payloadSize)) {
        VPN_SERVER_LOGE("❌ [转发-新] 无法提取payload");
        return -1;
    }
    
    if (payloadSize <= 0) {
        VPN_SERVER_LOGI("ℹ️ [转发-新] payload为空（可能是TCP控制包），跳过");
        return 0;  // TCP SYN/ACK等控制包没有payload，这是正常的
    }
    
    VPN_SERVER_LOGI("✅ [转发-新] 提取payload: %{public}d字节", payloadSize);
    
    // 2. 检查是否已有NAT映射（用于连接复用）
    std::string natKey = NATTable::GenerateKey(packetInfo);
    NATConnection existingConn;
    
    if (NATTable::FindMapping(natKey, existingConn)) {
        // 连接已存在，复用socket
        VPN_SERVER_LOGI("♻️ [转发-新] 复用现有连接: socket=%{public}d", existingConn.forwardSocket);
        
        // 更新活动时间
        NATTable::UpdateActivity(natKey);
        
        // 直接发送payload
        ssize_t sent = send(existingConn.forwardSocket, payload, payloadSize, 0);
        if (sent > 0) {
            VPN_SERVER_LOGI("✅ [转发-新] 数据已发送: %{public}zd字节", sent);
            return existingConn.forwardSocket;
        } else {
            VPN_SERVER_LOGE("❌ [转发-新] 发送失败，移除映射");
            NATTable::RemoveMapping(natKey);
            close(existingConn.forwardSocket);
        }
    }
    
    // 3. 创建新连接
    int addressFamily = (packetInfo.targetIP.find(':') != std::string::npos) ? AF_INET6 : AF_INET;
    int sockFd = CreateSocket(addressFamily, packetInfo.protocol);
    if (sockFd < 0) {
        VPN_SERVER_LOGE("❌ [转发-新] 创建socket失败");
        return -1;
    }
    
    // 4. 根据协议转发
    int result = -1;
    if (packetInfo.protocol == PROTOCOL_UDP) {
        result = HandleUDPForwarding(sockFd, payload, payloadSize, packetInfo, addressFamily, originalPeer);
    } else if (packetInfo.protocol == PROTOCOL_TCP) {
        result = HandleTCPForwarding(sockFd, payload, payloadSize, packetInfo, addressFamily, originalPeer);
    } else {
        VPN_SERVER_LOGE("❌ [转发-新] 不支持的协议: %{public}d", packetInfo.protocol);
        close(sockFd);
        return -1;
    }
    
    if (result < 0) {
        VPN_SERVER_LOGE("❌ [转发-新] 转发失败");
        close(sockFd);
        return -1;
    }
    
    // 5. 创建NAT映射
    NATTable::CreateMapping(natKey, originalPeer, packetInfo, result);
    
    VPN_SERVER_LOGI("✅ [转发-新] 转发成功, socket=%{public}d", result);
    return result;
}

// 创建socket
int PacketForwarder::CreateSocket(int addressFamily, uint8_t protocol) {
    int sockType = (protocol == PROTOCOL_UDP) ? SOCK_DGRAM : SOCK_STREAM;
    int sockFd = socket(addressFamily, sockType, 0);
    
    if (sockFd < 0) {
        VPN_SERVER_LOGE("❌ [创建Socket] 失败: %{public}s", strerror(errno));
        return -1;
    }
    
    // 设置socket选项
    int optval = 1;
    setsockopt(sockFd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
    
    // 设置非阻塞模式
    int flags = fcntl(sockFd, F_GETFL, 0);
    if (flags >= 0) {
        fcntl(sockFd, F_SETFL, flags | O_NONBLOCK);
    }
    
    VPN_SERVER_LOGI("✅ [创建Socket] 成功, fd=%{public}d, 地址族=%{public}d, 类型=%{public}d", 
                    sockFd, addressFamily, sockType);
    return sockFd;
}

// 处理UDP转发 - 重构版（只转发payload）
int PacketForwarder::HandleUDPForwarding(int sockFd, const uint8_t* payload, int payloadSize,
                                        const PacketInfo& packetInfo,
                                        int addressFamily, const sockaddr_in& originalPeer) {
    VPN_SERVER_LOGI("📮 [UDP-新] 开始: 目标=%{public}s:%{public}d, payload=%{public}d字节",
                    packetInfo.targetIP.c_str(), packetInfo.targetPort, payloadSize);
    
    // 构建目标地址
    struct sockaddr_in serverAddr{};
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(packetInfo.targetPort);
    
    if (inet_pton(AF_INET, packetInfo.targetIP.c_str(), &serverAddr.sin_addr) <= 0) {
        VPN_SERVER_LOGE("❌ [UDP-新] 无效的IP地址: %{public}s", packetInfo.targetIP.c_str());
        return -1;
    }
    
    // 发送payload（应用层数据），操作系统会自动添加UDP和IP头
    ssize_t sent = sendto(sockFd, payload, payloadSize, 0, 
                          (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    
    if (sent < 0) {
        VPN_SERVER_LOGE("❌ [UDP-新] 发送失败: %{public}s", strerror(errno));
        return -1;
    }
    
    VPN_SERVER_LOGI("✅ [UDP-新] payload已发送: %{public}zd字节 (OS会自动添加UDP/IP头)", sent);
    
    if (IsDNSQuery(packetInfo.targetIP, packetInfo.targetPort)) {
        VPN_SERVER_LOGI("🔍 [DNS查询] 检测到DNS查询");
    }
    
    // 启动UDP响应处理线程
    std::thread(&PacketForwarder::HandleUdpResponse, sockFd, originalPeer, packetInfo).detach();
    
    return sockFd;
}

// 处理TCP转发 - 重构版（只转发payload）
int PacketForwarder::HandleTCPForwarding(int sockFd, const uint8_t* payload, int payloadSize,
                                        const PacketInfo& packetInfo,
                                        int addressFamily, const sockaddr_in& originalPeer) {
    VPN_SERVER_LOGI("🔗 [TCP-新] 开始: 目标=%{public}s:%{public}d, payload=%{public}d字节",
                    packetInfo.targetIP.c_str(), packetInfo.targetPort, payloadSize);
    
    // 构建目标地址
    struct sockaddr_in serverAddr{};
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(packetInfo.targetPort);
    
    if (inet_pton(AF_INET, packetInfo.targetIP.c_str(), &serverAddr.sin_addr) <= 0) {
        VPN_SERVER_LOGE("❌ [TCP-新] 无效的IP地址: %{public}s", packetInfo.targetIP.c_str());
        return -1;
    }
    
    // 设置socket为阻塞模式（TCP连接需要阻塞）
    int flags = fcntl(sockFd, F_GETFL, 0);
    fcntl(sockFd, F_SETFL, flags & ~O_NONBLOCK);
    
    // 连接目标服务器
    VPN_SERVER_LOGI("🔌 [TCP-新] 正在连接 %{public}s:%{public}d...", 
                    packetInfo.targetIP.c_str(), packetInfo.targetPort);
    
    int connectResult = connect(sockFd, (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    
    if (connectResult < 0) {
        VPN_SERVER_LOGE("❌ [TCP-新] 连接失败: %{public}s", strerror(errno));
        return -1;
    }
    
    VPN_SERVER_LOGI("✅ [TCP-新] 连接成功");
    
    // 发送payload（应用层数据），操作系统会自动添加TCP和IP头
    if (payloadSize > 0) {
        ssize_t sent = send(sockFd, payload, payloadSize, 0);
        
        if (sent < 0) {
            VPN_SERVER_LOGE("❌ [TCP-新] 发送失败: %{public}s", strerror(errno));
            return -1;
        }
        
        VPN_SERVER_LOGI("✅ [TCP-新] payload已发送: %{public}zd字节 (OS会自动添加TCP/IP头)", sent);
    } else {
        VPN_SERVER_LOGI("ℹ️ [TCP-新] 无payload（TCP握手包）");
    }
    
    // 启动TCP响应处理线程
    std::thread(&PacketForwarder::HandleTcpResponse, sockFd, originalPeer, packetInfo).detach();
    
    return sockFd;
}

// 处理UDP响应 - 重构版（重新封装成IP包）
void PacketForwarder::HandleUdpResponse(int sockFd, sockaddr_in originalPeer, const PacketInfo& packetInfo) {
    VPN_SERVER_LOGI("📥 [UDP响应-新] 开始监听: socket=%{public}d", sockFd);
    
    // 查找NAT映射
    NATConnection conn;
    if (!NATTable::FindMappingBySocket(sockFd, conn)) {
        VPN_SERVER_LOGE("❌ [UDP响应-新] 未找到NAT映射");
        close(sockFd);
        return;
    }
    
    uint8_t responsePayload[65535];  // 应用层响应数据
    struct sockaddr_in responseAddr{};
    socklen_t addrLen = sizeof(responseAddr);
    
    // 设置接收超时（30秒）
    struct timeval timeout;
    timeout.tv_sec = 30;
    timeout.tv_usec = 0;
    setsockopt(sockFd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
    
    while (true) {
        // 接收响应（应用层数据，不包含IP/UDP头）
        ssize_t received = recvfrom(sockFd, responsePayload, sizeof(responsePayload), 0,
                                    (struct sockaddr*)&responseAddr, &addrLen);
        
        if (received <= 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                VPN_SERVER_LOGI("⏱️ [UDP响应-新] 接收超时");
            } else {
                VPN_SERVER_LOGE("❌ [UDP响应-新] 接收失败: %{public}s", strerror(errno));
            }
            break;
        }
        
        VPN_SERVER_LOGI("📬 [UDP响应-新] 收到payload: %{public}zd字节", received);
        
        if (IsDNSQuery(packetInfo.targetIP, packetInfo.targetPort)) {
            VPN_SERVER_LOGI("🔍 [DNS响应-新] 收到DNS响应");
        }
        
        // 重新封装成完整的IP数据包
        uint8_t ipPacket[65535];
        int packetLen = PacketBuilder::BuildResponsePacket(
            ipPacket, sizeof(ipPacket),
            responsePayload, received,
            conn.originalRequest
        );
        
        if (packetLen < 0) {
            VPN_SERVER_LOGE("❌ [UDP响应-新] 构建响应包失败");
            break;
        }
        
        VPN_SERVER_LOGI("📦 [UDP响应-新] 已构建完整IP包: %{public}d字节", packetLen);
        
        // 发送完整IP包给客户端
        ssize_t sent = sendto(g_sockFd, ipPacket, packetLen, 0,
                             (struct sockaddr*)&conn.clientPhysicalAddr, 
                             sizeof(conn.clientPhysicalAddr));
        
        if (sent > 0) {
            VPN_SERVER_LOGI("✅ [UDP响应-新] 完整IP包已发送给客户端: %{public}zd字节", sent);
        } else {
            VPN_SERVER_LOGE("❌ [UDP响应-新] 发送失败: %{public}s", strerror(errno));
        }
        
        // UDP通常是一次性请求/响应
        break;
    }
    
    // 清理NAT映射和socket
    std::string natKey = NATTable::GenerateKey(conn.originalRequest);
    NATTable::RemoveMapping(natKey);
    close(sockFd);
    VPN_SERVER_LOGI("🔒 [UDP响应-新] 关闭socket并移除映射: %{public}d", sockFd);
}

// 处理TCP响应 - 重构版（重新封装成IP包）
void PacketForwarder::HandleTcpResponse(int sockFd, sockaddr_in originalPeer, const PacketInfo& packetInfo) {
    VPN_SERVER_LOGI("📥 [TCP响应-新] 开始监听: socket=%{public}d", sockFd);
    
    // 查找NAT映射
    NATConnection conn;
    if (!NATTable::FindMappingBySocket(sockFd, conn)) {
        VPN_SERVER_LOGE("❌ [TCP响应-新] 未找到NAT映射");
        close(sockFd);
        return;
    }
    
    uint8_t responsePayload[8192];  // 应用层响应数据
    
    // 设置接收超时（60秒）
    struct timeval timeout;
    timeout.tv_sec = 60;
    timeout.tv_usec = 0;
    setsockopt(sockFd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
    
    while (true) {
        // 接收响应（应用层数据，不包含IP/TCP头）
        ssize_t received = recv(sockFd, responsePayload, sizeof(responsePayload), 0);
        
        if (received <= 0) {
            if (received == 0) {
                VPN_SERVER_LOGI("🔚 [TCP响应-新] 连接关闭");
            } else if (errno == EAGAIN || errno == EWOULDBLOCK) {
                VPN_SERVER_LOGI("⏱️ [TCP响应-新] 接收超时");
            } else {
                VPN_SERVER_LOGE("❌ [TCP响应-新] 接收失败: %{public}s", strerror(errno));
            }
            break;
        }
        
        VPN_SERVER_LOGI("📬 [TCP响应-新] 收到payload: %{public}zd字节", received);
        
        // 更新活动时间
        std::string natKey = NATTable::GenerateKey(conn.originalRequest);
        NATTable::UpdateActivity(natKey);
        
        // 重新封装成完整的IP数据包
        uint8_t ipPacket[8192 + 60];  // payload + IP头 + TCP头
        int packetLen = PacketBuilder::BuildResponsePacket(
            ipPacket, sizeof(ipPacket),
            responsePayload, received,
            conn.originalRequest
        );
        
        if (packetLen < 0) {
            VPN_SERVER_LOGE("❌ [TCP响应-新] 构建响应包失败");
            break;
        }
        
        VPN_SERVER_LOGI("📦 [TCP响应-新] 已构建完整IP包: %{public}d字节", packetLen);
        
        // 发送完整IP包给客户端
        ssize_t sent = sendto(g_sockFd, ipPacket, packetLen, 0,
                             (struct sockaddr*)&conn.clientPhysicalAddr,
                             sizeof(conn.clientPhysicalAddr));
        
        if (sent > 0) {
            VPN_SERVER_LOGI("✅ [TCP响应-新] 完整IP包已发送给客户端: %{public}zd字节", sent);
        } else {
            VPN_SERVER_LOGE("❌ [TCP响应-新] 发送失败: %{public}s", strerror(errno));
            break;
        }
    }
    
    // 清理NAT映射和socket
    std::string natKey = NATTable::GenerateKey(conn.originalRequest);
    NATTable::RemoveMapping(natKey);
    close(sockFd);
    VPN_SERVER_LOGI("🔒 [TCP响应-新] 关闭socket并移除映射: %{public}d", sockFd);
}

// 检查是否为DNS查询
bool PacketForwarder::IsDNSQuery(const std::string& targetIP, int targetPort) {
    // DNS通常使用端口53
    if (targetPort != 53) {
        return false;
    }
    
    // 常见的DNS服务器IP
    return (targetIP == "8.8.8.8" || 
            targetIP == "8.8.4.4" || 
            targetIP == "1.1.1.1" ||
            targetIP == "114.114.114.114" ||
            targetIP == "223.5.5.5" ||
            targetIP == "223.6.6.6");
}

// 测试网络连接 - 简化版本
bool PacketForwarder::TestNetworkConnectivity() {
    VPN_SERVER_LOGI("🔍 [网络测试] 开始简单的网络测试...");
    
    // 异步执行，避免阻塞主线程
    std::thread([]() {
        int sock = socket(AF_INET, SOCK_STREAM, 0);
        if (sock < 0) {
            VPN_SERVER_LOGE("❌ [网络测试] socket创建失败");
            return;
        }
        
        // 测试连接百度 110.242.68.66:80
        struct sockaddr_in addr{};
        addr.sin_family = AF_INET;
        addr.sin_port = htons(80);
        inet_pton(AF_INET, "110.242.68.66", &addr.sin_addr);
        
        // 简单的阻塞式连接测试（在后台线程，不影响主线程）
        int result = connect(sock, (struct sockaddr*)&addr, sizeof(addr));
        
        if (result == 0) {
            VPN_SERVER_LOGI("✅ [网络测试] 网络连接正常");
        } else {
            VPN_SERVER_LOGE("❌ [网络测试] 网络连接失败");
        }
        
        close(sock);
    }).detach();
    
    return true;
}
